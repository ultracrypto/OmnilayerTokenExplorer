...
namespace BitcoinLib.Services
{
    ...
    public partial class CoinService : ICoinService
    {
        ...
        public virtual List<MasternodeFullResponse> MasternodesList()
        {
            var resp = _rpcConnector.MakeRequest<Dictionary<string, string>>(RpcMethods.masternodelist, "full");
            List<MasternodeFullResponse> lst = new List<MasternodeFullResponse>();
            foreach (KeyValuePair<string, string> kv in resp)
            {
                MasternodeFullResponse mr = new MasternodeFullResponse(kv.Key, kv.Value);
                lst.Add(mr);
            }
            return lst;
        }

        public virtual List<MasternodeRemoteResponse> MasternodesRemoteList()
        {
            var resp = _rpcConnector.MakeRequest<Dictionary<string, string>>(RpcMethods.masternodelist, "reward");
            List<MasternodeRemoteResponse> lst = new List<MasternodeRemoteResponse>();
            foreach (KeyValuePair<string, string> kv in resp)
            {
                MasternodeRemoteResponse mr = new MasternodeRemoteResponse(kv.Key, kv.Value);
                lst.Add(mr);
            }
            return lst;
        }
        ...
        public virtual GetBlockResponse GetBlock(string hash, bool verbose)
        {
        ...
        public string GetRawJsonQuery(string query, params object[] parameters)
        {
            return _rpcConnector.MakeRawRequest(query, parameters);
        }
        ...
        public GetInfoResponseMoney GetInfoMoney()
        {
            return _rpcConnector.MakeRequest<GetInfoResponseMoney>(RpcMethods.getinfo);
        }

        public GetInfoResponseLight GetInfoLight()
        {
            return _rpcConnector.MakeRequest<GetInfoResponseLight>(RpcMethods.getinfo);
        }
        ...
        public virtual GetMiningInfoResponse GetMiningInfo()
        {
            var mining =  _rpcConnector.MakeRequest<GetMiningInfoResponse>(RpcMethods.getmininginfo);
            mining.ProofOfWork = mining.Difficulty;
            return mining;
        }
        ...
        public GetNetworkInfoResponseLight GetNetworkInfoLight()
        {
            return _rpcConnector.MakeRequest<GetNetworkInfoResponseLight>(RpcMethods.getnetworkinfo);
        }
        ...
        public virtual List<string> GetRawMemPool2()
        {
            return _rpcConnector.MakeRequest<List<string>>(RpcMethods.getrawmempool);
        }

        public virtual GetRawMemPoolResponse GetRawMemPool(bool verbose)
        {
                ...
                foreach (var property in children[i].SelectMany(grandChild => grandChild.OfType<JProperty>()))
                {
                    switch (property.Name)
                    {
                        ...
                        case "fee":
                            ...
                            if (decimal.TryParse(property.Value.ToString(), NumberStyles.Any, CultureInfo.InvariantCulture, out fee))
                            {
        ...
        public List<GetBudgetResponse> GetBudgetInfo()
        {
            return _rpcConnector.MakeRequest<List<GetBudgetResponse>>(RpcMethods.getbudgetinfo);
        }

        public List<GetBudgetResponse> GetBudgetProjection()
        {
            return _rpcConnector.MakeRequest<List<GetBudgetResponse>>(RpcMethods.getbudgetprojection);
        }

        public List<GetBudgetVotesResponse> GetBudgetVotes(string budgetName)
        {
            return _rpcConnector.MakeRequest<List<GetBudgetVotesResponse>>(RpcMethods.getbudgetvotes, budgetName);
        }

        public long GetNextSuperBlock()
        {
            return _rpcConnector.MakeRequest<long>(RpcMethods.mnbudget, "nextblock");
        }
        ...
        public GetOmniTransactionResponse GetOmniTransaction(string txId)
        {
            return _rpcConnector.MakeRequest<GetOmniTransactionResponse>(RpcMethods.omni_gettransaction, txId);
        }

        public List<string> GetOmniListBlockTransactions(long index)
        {
            ...
            for (var i = 0; i < unstructuredResponse.Count; i++)
            {
                for (var j = 0; j < unstructuredResponse[i].Count; j++)
                {
                    if (unstructuredResponse[i][j].Count > 1)
                    {
                        ...
                        decimal balance;
                        decimal.TryParse(unstructuredResponse[i][j][1].ToString(), NumberStyles.Any, CultureInfo.InvariantCulture, out balance);
